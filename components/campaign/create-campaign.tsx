...
import { calculatePriorityScore, cn } from "@/lib/utils";
...
export const CampaignCreationDialog = ({
	...
	const form = useForm<CampaignFormValues>({
		...
		defaultValues: {
			...
			endDate: undefined,
			targetingRules: {
				lastPurchaseRange: undefined,
				totalSpentRange: undefined,
				riskScoreRange: undefined,
				tags: [],
			},
			sendingSchedule: {
				maxEmailsPerDay: undefined,
				preferredTimeOfDay: [],
				daysOfWeek: [],
				timezone: "UTC",
			},
	...
	// Watch campaign type to auto-update priority
	const watchCampaignType = form.watch("type");
	...
	useEffect(() => {
		if (watchCampaignType) {
			const priorityScore = calculatePriorityScore({
				...
				targetAudience: watchTargetAudience,
			});

			// Set recommended priority based on score
			let recommendedPriority: "URGENT" | "HIGH" | "MEDIUM" | "LOW";
			if (priorityScore >= 80) recommendedPriority = "URGENT";
			else if (priorityScore >= 60) recommendedPriority = "HIGH";
			else if (priorityScore >= 40) recommendedPriority = "MEDIUM";
			else recommendedPriority = "LOW";

			form.setValue("priority", recommendedPriority);
		}
	}, [watchCampaignType, watchTargetAudience]);
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>
										Priority is automatically suggested based on campaign type
										but can be adjusted.
									</FormDescription>
									<FormMessage />
								</FormItem>
				...
				return (
					<div className="space-y-6">
						<div className="grid gap-6 md:grid-cols-2">
							...
							{/* ... (rest of the review layout remains the same) */}
						</div>
					</div>
				);
		}
